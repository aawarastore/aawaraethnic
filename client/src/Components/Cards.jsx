    ...
    <>
      ...
      <ToastContainer />
          ...
          <div className='my-2  hidden  ss:flex gap-1'>
            {
              [{ XS: 34 }, { S: 36 }, { M: 38 }, { L: 40 }, { XL: 42 }, { XXL: 44 }].map((item, index) => {
                ...
                return (
                  <div onClick={() => setSize(brandSize)} key={value} style={{ backgroundColor: size == brandSize ? 'rgb(78 78 78 / 48%)' : 'white' }} className='py-1 px-2 rounded-full  cursor-pointer text-center border border-stone-300 ss:text-[11px]  text-[7px] uppercase'>{brandSize}</div>
                );
              })
            }
          </div>
        ...
        <div className='flex  justify-between mt-1 w-full '>
            <div className='bg-gradient-to-r hover:bg-gradient-to-t from-[#282828d2]  to-black  rounded-[0px]  text-center text-white  px-2 text-[15px] ss:text-[18px] py-[6px] w-full  cursor-pointer' onClick={() => buyProduct(productsdata.PRODUCT_id)} >Buy </div>
          </div>
      </div>
    </>
  )
